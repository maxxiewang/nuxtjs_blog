<template>
    <div >
        ...
        <el-row class="main-top">
            <div class="avatar">
                <!-- ELE框架提供的方法，el-tooltip 和下面el-upload配合使用 -->
                <el-tooltip content="点击上传头像" effect="light">
                    <el-upload
                        ...
                        >
                        <!-- {{userInfo.nickName}} ,这个图像渲染不需要，但如果不上传头像，框架会把这个值赋给头像里面，钉钉那种 -->
        ...
        <el-row >
            <el-tabs value="public" @tab-click="handleClick" >
                <el-tab-pane label="公开文章" name="public">
                    <article-list name="public" :page="query" :listData="articleList"
                    @fetch-data="findUserArticleList"/>
                </el-tab-pane>
                <el-tab-pane label="私密文章" name="nopublic">
                    <article-list name="nopublic" :page="query" :listData="nopublicList"
                    @fetch-data="findUserArticleList"/>
                </el-tab-pane>
                <el-tab-pane label="我的提问" name="question">
                    <question-list name="question" :page="query" :listData="questionList"
...
export default {
    ...
    methods: {
        ...
        async findUserQuestionList(paneName, current){
            ...
            const {data} = await this.$findQuestion(this.query)
            //总记录数
